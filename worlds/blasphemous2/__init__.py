...
class Blasphemous2World(World):
    ...
    def create_items(self):
        ...
        for item in item_list:
            ...
            if not self.options.add_penitence_rewards:
                if item.name in group_table["penitence"]:
                    count = 0
                elif item.name == "Tears of Atonement (800)":
                    count += 5
            else:
                if item.name == "Tears of Atonement (800)":
                    count -= 1
    ...
    def create_regions(self):
        ...
        for l in extracted_locations:
            ...
            if not self.options.shuffle_cherubs and l["name"] in location_flags["C"]:
                location.place_locked_item(self.create_item("Cherub"))
            elif l["name"] in locked_items.keys():
                location.place_locked_item(self.create_item(locked_items[l["name"]]))
